# 💧 Log de Regas e Manejo Nutricional

Este arquivo serve como o banco de dados de performance do solo e absorção de nutrientes.

# 📈 Tabela de Acompanhamento - Ciclo: Radical Juice

| Data | Vol (L) | E.C In | E.C Out | P.H | Runoff | Observações Técnicas |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 30/03 | 3.0 | 1.600 | 1.900 | 6.0 | 600ml | Pistilos inferiores +2cm; Top buds 90%. |
| 02/04 | 3.0 | 1.600 | 1.600 | 6.0 | - | Rega de manutenção. |
| 04/04 | 3.5 | 1.800* | 2.000* | 5.0 | ? | *Leitura em PPM mascarando overfert real. |
| 06/04 | **15.0** | **0.0** | **6.680** | **5.5** | **Farto** | **🚨 INCIDENTE:** Detectado Overfert severo, Flush de Emergência (Erro de escala PPM) |
| 06/04 | - | - | **2.172** | - | - | Status pós-flush: Solo resetado. |
| 06/04 | 3.0 | 0.0 | - | 6.2 | - | Apenas água pH 6.2 (Descanso). |
| 09/04 | 4.0 | 0.0 | 900 | 6.0 | 500ml | **Atual:** Forçar planta a consumir o excesso do solo, Solo limpo. Descanso pós-estresse. |
| 11/04 | 4.0 | 1.300 | **2.800** | 6.1 | 800ml | Retorno de Nutrientes (2g A + 2g B). Setup LED Dark Red. |
| 13/04 | 4.0 | 1.250 | **2.600** | 6.5 | 500ml | **Rega de Hoje:** Redução gradual do E.C. de saída. |
| 16/04 | 4.0 | **800** | **2.700** | **6.0** | 500ml | **HOJE:** Solo soltando sais. Planta em consumo alto. |
| 16/04 | 4.0 | 800 | 2.700 | 6.0 | 500ml | Planta em alto consumo. |
| 21/04 | 4.0 | 700 | 2.000 | 6.0 | 700ml | Estabilização pré-flush. |
| 23/04 | **4.5** | 700 | 1.500 | 6.0 | 1.5L | Flush Corretivo Sucesso. |
| 25/04 | 4.0 | 500 | 2.200 | 6.0 | 1.0L | **Ajuste:** LED 100% / 30cm. Monitorar estresse. |
| 27/04 | 4.0 | 0 (H2O) | 2.500 | 6.2 | 700ml | Início do flush final. Solo ainda carregado. |
| 29/04 | 4.5 | 0 (H2O) | 1.890 | 6.0 | 700ml | **Flush Ativo:** Redução drástica de sais (2500 -> 1890). |
| 02/05 | 4.5 | 0 (H2O) | **1.100** | 6.0 | 500ml | **Flush Consolidado:** Solo quase totalmente limpo (Meta <1000). |

**Análise Técnica (02/05):** A lavagem do substrato está sendo extremamente eficaz. A planta respondeu ao estresse térmico aumentando a densidade das flores e a produção de resina foliar.

**Status (29/04):** Planta iniciando consumo de reservas internas. Manter apenas água.

**Análise Técnica (27/04):** Mesmo com entrada de E.C 0, a saída subiu. Solo em processo de lixiviação. Manter apenas água.
---
**Análise Técnica (16/04):**
- Runoff de 2.700 mostra que a planta está "limpando" o solo.
- Pistilos oxidando rápido, indicando que a janela de colheita (10/05) está batendo.

---
**Próxima Rega (Prevista: 20/04):** Manter E.C baixo para limpar o meio.

---

## 🧪 Rega com Nutrientes (Em para 09/04)

Considerando que 5g em 3L resultou em um E.C altíssimo (> 3.000), vamos adotar a estratégia de **Titulação Reversa** para atingir o alvo de **1.200 a 1.400 µS/cm** (entrada leve pós-flush).

### 🛠️ Receita para 3 Litros de Água:
1. **Easycoco A:** 2.0g
2. **Easycoco B:** 2.0g
3. **Procedimento:** - Misture os 2g de cada e meça o E.C.
   - Se estiver abaixo de 1.200, adicione de 0.5g em 0.5g.
   - **Target final:** E.C 1.200 / pH 6.0.

---

## 📝 Notas de Engenharia (Post-Mortem)
- **Causa Raiz:** Confusão entre escalas PPM (0.5) e µS/cm (E.C).
- **Ação Corretiva:** Todas as medições agora são registradas estritamente em **µS/cm** para evitar novos desvios.
- **Status da Planta:** Observar redução das "patas de águia" após o reset do substrato.

------




## 🛠️ Guia de Preparo Rápido (Easycoco em Gramas)

Use esta tabela para preparar **3 Litros** de solução nutritiva:

| Alvo E.C | Part A | Part B | Part C | Objetivo |
| :--- | :--- | :--- | :--- | :--- |
| **0.0 (Flush)** | 0g | 0g | 0g | Limpeza de sais / Correção |
| **1.2 (Leve)** | 3.0g | 3.0g | 3.0g | Manutenção pós-stress |
| **1.6 (Médio)** | 4.0g | 4.0g | 4.0g | Engorda de buds (Atual) |
| **2.0 (Forte)** | 5.5g | 5.5g | 5.5g | Pico de floração |


## 📝 Notas de Manejo
**[23/03]:** E.C está em equilíbrio perfeito (In=Out). Manter a dose. A umidade de 65% (chuva em Brasília) está dificultando a transpiração.
- **Dica de QA:** Se o E.C de saída subir acima de 2.200, realizar rega apenas com água e CalMag na próxima vez.

**Detalhamento do Flush (23/04):**
- Fração 1 (500ml): 2.500 µS/cm
- Fração 2 (500ml): 3.200 µS/cm (Pico de sais acumulados)
- Fração 3 (250ml): 1.600 µS/cm
- **Média Final:** 1.500 µS/cm (Solo resetado para a fase final).
  

**[30/03]:** UR 55-60% / Temp 23-26ºC (Ambiente ideal). O E.C de saída subiu para 1.900 (acumulou um pouco desde o flush), mas está dentro da margem de segurança. 
- **QA Alert:** As "patas de águia" persistem; a planta ainda está processando o excesso de Nitrogênio anterior. Manter a rega em 1.600 de entrada para não sobrecarregar.

**[13/04]:**
- O E.C. de saída baixou 200 pontos, confirmando que a planta está em alto consumo metabólico.
- A proporção de runoff (12.5%) foi ideal para renovar a solução no substrato sem causar um flush agressivo.
- As fotos mostram os pistilos começando a oxidar (ficar marrons), sinal claro de maturação avançada.

**[24/04]:**
Com o solo limpo (E.C 1500) e o Ar Condicionado chegando dia 29, a planta terá o clima perfeito para os últimos 10 dias. 
- **Estratégia:** Provável transição para água pura (pH 6.0-6.2) a partir da próxima rega para consumir os nutrientes restantes e melhorar o sabor (terpenos).

---

💡 Dica Ninja:
Como você vai desligar às 21h, você vai ter aquela "janela" das 09:00 às 11:00 da manhã para fazer as regas com a luz recém-acesa (que é o horário que a planta mais absorve água). Além disso, às 21h você dá o "boa noite" pra elas antes de dormir.

---
*Última sincronização: 24/04/2026*